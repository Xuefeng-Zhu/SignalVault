...
import { seededInferenceText } from "@/lib/adapters/model/demo-inference";
 ...
 * stay pure: a recording client (to assert the request shape/stance), the
 * deterministic demo inference mapping (to assert real seeded output parses),
 * and a malformed client (to assert validation failures surface as
...
class RecordingModelClient implements ModelClient {
  ...
  async complete(req: InferenceRequest): Promise<{ text: string; simulated: boolean }> {
    ...
    return { text: this.responder(req), simulated: true };
  }
}

/** A ModelClient backed by the deterministic seeded demo inference mapping. */
const demoClient = new RecordingModelClient((req) => seededInferenceText(req));
...
describe("runDefense", () => {
  it("returns a schema-valid defense argument from seeded demo output", async () => {
    const result = await runDefense({ ...evidence, model: demoClient });
  ...
  it("builds a request with the defense-for stance and traced schema name", async () => {
    const client = new RecordingModelClient((req) => seededInferenceText(req));
...
describe("runProsecutor", () => {
  it("returns a schema-valid prosecutor argument from seeded demo output", async () => {
    const result = await runProsecutor({ ...evidence, model: demoClient });
