...
/**
 * Property 28 (Validates: Requirements 22.2, 22.3, 22.5):
 *
 * For ANY integration credential value, neither the value persisted at rest
 * nor the value transmitted to the browser exposes the credential:
 *
 *   - PERSISTED ≠ PLAINTEXT (Req 22.3): `buildStoredCredential` returns
 ...
 *  - It is structurally distinct from the mask (`••••••••`), so a generated
 *    token never collides with it.
 */
...
describe("Property 28: credentials never leak to the browser (Requirements 22.2, 22.3, 22.5)", () => {
  it("persisted value != plaintext and the browser response carries no secret", () => {
    fc.assert(
      fc.property(
        providerArb,
        tokenArb,
        ...
        (provider, token, secret) => {
          ...
          // Precondition: the plaintext is not the public mask.
          ...
          // (Req 22.3): the persisted value is encrypted and is never
