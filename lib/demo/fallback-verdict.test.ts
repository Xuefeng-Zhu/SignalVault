...
/**
 * The Demo_Company verdict is deterministic: strategy prediction "moving_upmarket"
 * with a confidence of 85 on a 0–100 scale (AI platform pivot).
 *
 * Validates: Requirements 18.5
 */
describe('Demo_Company seeded verdict (Requirement 18.5)', () => {
  it('predicts "moving_upmarket" with confidence 85', () => {
    expect(DEMO_VERDICT.strategyPrediction).toBe('moving_upmarket');
    expect(DEMO_VERDICT.confidence).toBe(85);
  });

  it('exposes the human-readable label "AI platform pivot"', () => {
    expect(DEMO_STRATEGY_LABEL).toBe('AI platform pivot');
  });

  it('keeps the strategy/confidence constants aligned with the verdict', () => {
    expect(DEMO_STRATEGY_PREDICTION).toBe('moving_upmarket');
    expect(DEMO_CONFIDENCE).toBe(85);
  ...
  it('mirrors the same conclusion in the deterministic fallback verdict', () => {
    ...
    expect(DEMO_FALLBACK_VERDICT.confidence).toBe(85);
